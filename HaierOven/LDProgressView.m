...
//        }
//        self.animationTimer = [NSTimer scheduledTimerWithTimeInterval:0.01 target:self selector:@selector(incrementAnimatingProgress) userInfo:nil repeats:YES];
//    } else {
//        _progress = progress;
...
- (void)setProgress:(CGFloat)progress andTimeInterval:(float)time{
    self.progressToAnimateTo = progress;
    if ([self.animate boolValue]) {
        if (self.animationTimer) {
            [self.animationTimer invalidate];
        }
        self.animationTimer = [NSTimer scheduledTimerWithTimeInterval:time target:self selector:@selector(incrementAnimatingProgress) userInfo:nil repeats:YES];
    } else {
        _progress = progress;
        [self setNeedsDisplay];
    }
